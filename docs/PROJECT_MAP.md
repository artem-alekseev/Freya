...
Снимок составлен по текущему коду 29 июля 2026 года. Это навигационный документ,
...
4. Регистрирует события, Lua, клиентские пакеты и обратные RPC handlers.
5. Подключается к Master и регистрирует канал.
6. Слушает клиентский TCP на настроенном порту.

Важно: поиск `cfg/`, `data/` и `script/` зависит от
...
4. `Inventory`, `Equipment` и `Links` получают RPC connection через
   `Setup(...)`.
5. `context.Context` получает выбранного персонажа, мир и клетку.
6. `World.EnterWorld` рассылает состояние соседним клеткам и запускает
...
| Инвентарь, экипировка, навыки, quick links | Game-модели + Master RPC | World DB |
| Миры, клетки, мобы, предметы на земле | Game `WorldManager` | В основном YAML/runtime |
...
- рабочее дерево уже содержит большой незавершённый набор пользовательских
  изменений вокруг боя, навыков, мобов, инвентаря, пакетов и конфигурации;
- `cmd/masterserver/rpc/skill.go` является новым untracked-файлом;
- тестовых файлов `*_test.go` нет;
- `go test ./...` успешно компилирует все пакеты; автоматических тестов пока
  нет, поэтому runtime-поведение `UpgradeSkill` ещё требует проверки;
- при `Initialized` модели `Inventory`, `Equipment` и `Links` получают
  `ChannelId`, хотя Master использует поле `Server` запроса как индекс World
  DB; ошибка скрыта при паре `serverID=1/channelID=1`, но опасна для нескольких
  каналов;
...
- спавн мобов временно управляется параметром `[server] spawn_mobs`; в
  `cfg/gameserver_1_1.ini` он выключен для диагностики вылета клиента;
...
После завершения текущей ветки его следует обновить.
