...
15. `Initialized` передаёт клиенту список активных квестов с UI-флагами и
   нулевыми счётчиками миссий, используя количество миссий из `quest.dec`.
  16. `QuestNPCAction` (`320`) разбирает NPC-действия из `quest.dec`, обрабатывает
     выдачу/изъятие предметов через `Inventory` и возвращает клиенту `wFlagNPC`.
  17. `QuestCloseEvent` (`283`) читает выбранные слоты `bData`, выдает награду
     из `quest.dec`, сохраняет опыт/Alz и удаляет завершенный квест через Master RPC.
  18. `World.EnterWorld` рассылает состояние соседним клеткам и запускает
     игровые события/Lua hooks.
18. При переходе в новую клетку, варпе, выходе из мира или отключении Game
